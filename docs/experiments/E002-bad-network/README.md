...
- выявления backpressure и хвостовых задержек,
...
## Описание CSV-таблиц и столбцов

Ниже приведено описание основных CSV-файлов, формируемых на этапе агрегации, и назначение их столбцов. Эти таблицы являются каноническим представлением результатов эксперимента.

---

### `profile_table.csv`

**Назначение:** описание сетевого профиля и окружения эксперимента.

**1 строка = 1 сетевой профиль (один прогон).**

|Столбец|Описание|
|---|---|
|`profile_id`|Идентификатор профиля (имя каталога прогона)|
|`run_dir`|Путь к каталогу с результатами прогона|
|`iface`|Сетевой интерфейс, на котором применялся netem|
|`usbip_target`|Адрес и порт USB/IP-сервера|
|`kernel`|Версия ядра Linux на клиенте|
|`fio_version`|Версия fio|
|`mount_line`|Строка монтирования USB-устройства|
|`delay_ms`|Инъецированная сетевая задержка (мс)|
|`jitter_ms`|Инъецированный джиттер (мс), если применялся|
|`loss_pct`|Процент потерь пакетов (если применялся)|
|`netem_limit_pkts`|Лимит очереди netem (пакеты)|
|`qdisc_qdisc_type`|Тип qdisc (ожидается `netem`)|
|`qdisc_qdisc_limit_pkts`|Размер очереди qdisc|
|`qdisc_qdisc_sent_bytes`|Отправлено байт через qdisc|
|`qdisc_qdisc_sent_pkts`|Отправлено пакетов|
|`qdisc_qdisc_dropped_pkts`|Количество дропов|

---

### `fio_table.csv`

**Назначение:** результаты выполнения fio-нагрузок.

**1 строка = профиль × workload × направление I/O.**

|Столбец|Описание|
|---|---|
|`profile_id`|Идентификатор сетевого профиля|
|`jobname`|Имя fio job (`rand_read_4k`, `seq_write` и т.д.)|
|`rw`|Направление операции (`read` / `write`)|
|`bs`|Размер блока I/O|
|`runtime_ms`|Фактическое время выполнения теста|
|`iops`|Среднее количество операций ввода-вывода в секунду|
|`bw_kib_s`|Пропускная способность (KiB/s)|
|`bw_bytes_s`|Пропускная способность (байт/с)|
|`total_ios`|Общее количество завершённых операций|
|`lat_mean_ms`|Средняя латентность операции|
|`lat_max_ms`|Максимальная латентность|
|`slat_mean_ms`|Средняя задержка отправки I/O (submission latency)|
|`slat_max_ms`|Максимальная slat|
|`clat_mean_ms`|Средняя задержка выполнения I/O (completion latency)|
|`clat_max_ms`|Максимальная clat|
|`clat_p50_ms`|p50 latency|
|`clat_p95_ms`|p95 latency|
|`clat_p99_ms`|p99 latency|
|`clat_p99_9_ms`|p99.9 latency|
|`source_file`|Путь к исходному fio JSON-файлу|

---

### `net_table.csv`

**Назначение:** сетевые и TCP-метрики, зафиксированные во время прогона.
...
|`tcp_rtt_ms`|Оценка RTT TCP-соединения|
|`tcp_rtt_var_ms`|Дисперсия RTT|
|`tcp_rto_ms`|TCP retransmission timeout|
|`tcp_cwnd`|Размер congestion window|
|`tcp_bytes_retrans`|Количество повторно переданных байт|
|`tcp_retrans_total`|Количество ретрансляций|
|`TcpRetransSegs`|Число ретранслированных TCP-сегментов|
|`TcpTimeouts`|Количество TCP timeout|
|`TcpExtDelayedACKLost`|Потерянные delayed ACK|

---

### `summary.csv`

**Назначение:** объединённая таблица для анализа и построения графиков.

`summary.csv` формируется путём объединения:
...
Используется как основной источник данных для:

- построения графиков,
    
- анализа деградации,
    
- сравнения USB/IP и USB-over-QUIC.
